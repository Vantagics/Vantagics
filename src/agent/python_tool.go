...
`, workDir, encodedCode)
	...
	// Check for all image files (PNG, JPG, JPEG, SVG, GIF)
	// Support multiple chart files generated by analysis (e.g., rfm_clusters.png, rfm_comparison.png)
	var imageFiles []string
	imageExtensions := []string{"*.png", "*.jpg", "*.jpeg", "*.svg", "*.gif"}
	for _, ext := range imageExtensions {
		matches, _ := filepath.Glob(filepath.Join(workDir, ext))
		imageFiles = append(imageFiles, matches...)
	}
	...
	if len(imageFiles) > 0 {
		...
		for _, imagePath := range imageFiles {
			...
			fileInfo, statErr := os.Stat(imagePath)
			if statErr != nil {
				continue
			}
			if fileInfo.Size() > maxImageSize {
				output += fmt.Sprintf("\n\n⚠️ **Chart file too large (skipped):** `%s` (%.2f MB)\n", 
			...
			savedName, saveErr := saveToSession(imagePath, fileName, "image")
			if saveErr == nil && savedName != "" {
				// Reference the saved file with a friendly name
				imageCount++
				chartTitle := strings.TrimSuffix(fileName, filepath.Ext(fileName))
				chartTitle = strings.ReplaceAll(chartTitle, "_", " ")
				chartTitle = strings.Title(chartTitle)
				output += fmt.Sprintf("\n\n📊 **Chart %d - %s:** `files/%s`\n", imageCount, chartTitle, savedName)
			}
			...
			output += fmt.Sprintf("![Chart](data:%s;base64,%s)", mimeType, encoded)
		}
		
		if imageCount > 0 {
			output += fmt.Sprintf("\n\n✅ **Total charts generated:** %d\n", imageCount)
		}
	}
