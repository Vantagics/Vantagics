...
func (c *LicenseClient) RequestOpenSourceSN(serverURL, email string) (*RequestSNResponse, error) {
	...
	client := &http.Client{Timeout: 30 * time.Second}
	resp, err := client.Post(serverURL+"/request-oss-sn", "application/json", bytes.NewBuffer(reqBody))
	if err != nil {
		return nil, fmt.Errorf("%s", i18n.T("license_client.connect_failed", err))
	}
	defer resp.Body.Close()
	...
	if c.log != nil {
		if result.Success {
			c.log(fmt.Sprintf("[LICENSE] Open source SN requested successfully for email: %s", email))
		} else {
			c.log(fmt.Sprintf("[LICENSE] Open source SN request failed: %s", result.Message))
		}
	}
