...
- Results are cached per session â€?no penalty for calling again
...
func (t *DataSourceContextTool) InvokableRun(ctx context.Context, input string, opts ...tool.Option) (string, error) {
	...
	if t.workingContextManager != nil {
		if cached, found := t.workingContextManager.GetCachedSchema(in.DataSourceID); found {
			...
			return "ðŸ“‹ [Using cached schema - no need to call again]\n\n" + cached, nil
		}
	}
	...
	if len(in.TableNames) > 0 {
		...
		if len(allTables) > 5 {
			...
			sb.WriteString(fmt.Sprintf("\nâš ï¸ Database has %d tables. Call this tool with specific 'table_names' to see details.\n\n", len(allTables)))
	...
	if len(relationships) > 0 {
		sb.WriteString("### ðŸ”— Detected Relationships (for JOIN)\n")
	...
	sb.WriteString("### ðŸ“Š Table Details\n\n")
			...
			if strings.HasSuffix(lowerCol, "_id") && lowerCol != "id" {
				...
				if tableSet[refTable] {
					relationships = append(relationships,
						fmt.Sprintf("%s.%s â†?%s.id (FK)", tableName, col, refTable))
				} else if tableSet[refTable+"s"] {
					relationships = append(relationships,
						fmt.Sprintf("%s.%s â†?%ss.id (FK)", tableName, col, refTable))
				} else if tableSet[refTable+"es"] {
					relationships = append(relationships,
						fmt.Sprintf("%s.%s â†?%ses.id (FK)", tableName, col, refTable))
				}
			}
		}
	}
	...
	for col, val := range sample {
		...
		if strings.Contains(lowerCol, "date") || strings.Contains(lowerCol, "time") ||
			...
			if len(strVal) == 8 && isNumeric(strVal) {
				hints = append(hints, fmt.Sprintf("âš ï¸ `%s`: Date format is YYYYMMDD (e.g., %s)", col, strVal))
			} else if strings.Contains(strVal, "-") {
				hints = append(hints, fmt.Sprintf("â„¹ï¸ `%s`: Date format is YYYY-MM-DD", col))
			} else if strings.Contains(strVal, "/") {
				hints = append(hints, fmt.Sprintf("âš ï¸ `%s`: Date format uses slashes", col))
			}
		}
	}
...
func (t *DataSourceContextTool) getSQLDialectHints(dbType string) string {
	...
	case "duckdb":
		return `
âš ï¸ SQL Dialect: DuckDB (Local Storage) - Use these syntax rules:
â€?Date functions: YEAR(date_col), MONTH(date_col), DAY(date_col) are natively supported
â€?Date format: strftime(date_col, '%Y-%m') or format(date_col, 'yyyy-mm')
â€?String concat: CONCAT(col1, ' ', col2) or col1 || ' ' || col2
â€?Window functions: Fully supported (ROW_NUMBER, RANK, etc.)
â€?Most MySQL and PostgreSQL functions are supported natively
â€?SUBSTR(str, start, len) or SUBSTRING(str, start, len)
â€?Current date: now(), today(), current_date
â€?CAST(col AS INTEGER/DOUBLE/VARCHAR) for type conversion
`
	case "mysql", "doris":
		return `
âš ï¸ SQL Dialect: MySQL/Doris - Use these syntax rules:
â€?Date functions: YEAR(date_col), MONTH(date_col), DAY(date_col)
â€?Date format: DATE_FORMAT(date_col, '%Y-%m') for YYYY-MM format
â€?String concat: CONCAT(col1, ' ', col2)
â€?IFNULL(a, b) or COALESCE(a, b)
â€?SUBSTRING(str, start, len) for substring
â€?Current date: NOW(), CURDATE()
â€?CAST(col AS SIGNED/DECIMAL/CHAR) for type conversion
â€?GROUP_CONCAT() for aggregating strings
`
	default:
		return ""
	}
...
func (t *DataSourceContextTool) GetFullSchemaHint(dataSourceID string, tableNames []string) string {
	...
	return fmt.Sprintf("\nðŸ’¡ TIP: To avoid multiple calls, get schema for all relevant tables at once:\n"+
