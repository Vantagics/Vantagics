...
func (l *Logger) Init(logDir string) error {
	...
	pattern := filepath.Join(actualLogDir, fmt.Sprintf("vantagics_%s_*.log", dateStr))
	matches, _ := filepath.Glob(pattern)
	runCount := len(matches) + 1
	filename := filepath.Join(actualLogDir, fmt.Sprintf("vantagics_%s_%d.log", dateStr, runCount))
...
func (l *Logger) checkAndRotate() {
	...
	newFilename := filepath.Join(l.logDir, fmt.Sprintf("vantagics_%s_%s.log", dateStr, timeStr))
