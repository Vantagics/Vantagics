...
if "%SKIP_BACKUP%"=="false" (
    ...
    if exist "%BACKEND_DIR%\vantagics.exe" (
        copy "%BACKEND_DIR%\vantagics.exe" "%PROJECT_ROOT%backup\vantagics_backup.exe" >nul
...
if not exist "%BACKEND_DIR%\vantagics.exe" call :error "Deployment failed - binary not found"
...
echo.
echo ## Files Modified
echo - `src\vantagics.exe` - Updated application binary
...
if exist "%PROJECT_ROOT%backup\vantagics_backup.exe" (
    call :log "Restoring previous binary..."
    copy "%PROJECT_ROOT%backup\vantagics_backup.exe" "%BACKEND_DIR%\vantagics.exe" >nul
