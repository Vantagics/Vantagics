...
- 试用版客户端每小时自动向服务器上报 Credits 使用量，确保数据同步。
...
- 市场使用 OAuth 社交账户登录，无需独立注册。
- 支持的登录方式：Google、Apple、Facebook、Amazon。
...
- 管理员可创建自定义分类。
- 每个分析包显示：名称、描述、数据源名称、分类、作者、定价模式。
- **免费包**：直接下载。
- **收费包**：需要使用市场 Credits 购买后下载。
- 下载完成后，.qap 文件保存到本地，可通过数据源右键菜单导入。

### 9.4 分享分析包

1. 在快捷包列表中右键点击分析包 → 选择「分享到市场」。
2. 如未登录，系统弹出 OAuth 登录界面。
3. 登录后显示分享设置对话框：
   - 选择分类。
   - 选择定价模式（免费或收费）。
   - 收费模式需输入 Credits 价格。
