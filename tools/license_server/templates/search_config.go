    ...
    <div style="display:flex;gap:20px;flex-wrap:wrap;">
        <div class="card" style="flex:0 0 280px;">
            <div class="card-header">
                <h2 class="card-title">搜索分组</h2>
                <button onclick="showSearchGroupForm()" class="btn btn-success btn-sm">+ 添加</button>
            </div>
            <div id="search-groups-list"></div>
        </div>
        <div class="card" style="flex:1;min-width:400px;">
            <div class="card-header">
                <h2 class="card-title">搜索引擎配置</h2>
                <div class="flex items-center gap-2">
                    <select id="search-config-group-filter" onchange="loadSearchConfigs()" class="form-select" style="width:auto">
                        <option value="">全部分组</option>
                        <option value="none">默认(无组)</option>
                    </select>
                    <button onclick="showSearchForm()" class="btn btn-success btn-sm">+ 添加配置</button>
                </div>
            </div>
...
function loadSearchGroups() {
    fetch('/api/search-groups').then(function(resp) { return resp.json(); }).then(function(data) {
        ...
        if (!searchGroups || searchGroups.length === 0) { 
            ...
            var html = '';
            searchGroups.forEach(function(g, idx) { 
                ...
                html += '<button data-action="edit-search-group" data-idx="' + idx + '" class="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs">编辑</button>';
                html += '<button data-action="delete-search-group" data-idx="' + idx + '" class="px-2 py-1 bg-red-100 text-red-700 rounded text-xs">删除</button>';
                html += '</div></div>'; 
            });
            list.innerHTML = html;
        }
        ...
        if (filterSelect) {
            ...
            searchGroups.forEach(function(g) { opts += '<option value="' + g.id + '">' + escapeHtml(g.name) + '</option>'; });
...
// Store search configs for event delegation
var searchConfigsData = [];
...
function loadSearchConfigs() {
    ...
    fetch('/api/search').then(function(resp) { return resp.json(); }).then(function(configs) {
        ...
        searchConfigsData = filteredConfigs;
        ...
        filteredConfigs.forEach(function(c, idx) {
            ...
            if (groupName) html += '<span class="ml-2 px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded">' + escapeHtml(groupName) + '</span>';
            html += '<p class="text-sm text-slate-500">类型: ' + escapeHtml(c.type) + '</p>';
            html += '<p class="text-xs text-slate-400">有效期: ' + (c.start_date || '无限制') + ' ~ ' + (c.end_date || '永久') + '</p></div>';
            html += '<div class="flex gap-2">';
            html += '<button data-action="edit-search" data-idx="' + idx + '" class="px-3 py-1 bg-blue-100 text-blue-700 rounded text-sm">编辑</button>';
            html += '<button data-action="delete-search" data-idx="' + idx + '" class="px-3 py-1 bg-red-100 text-red-700 rounded text-sm">删除</button>';
            html += '</div></div>';
        });
        list.innerHTML = html;
    });
...
// Event delegation for search groups
document.getElementById('search-groups-list').addEventListener('click', function(e) {
    var btn = e.target.closest('button[data-action]');
    if (!btn) return;
    var action = btn.getAttribute('data-action');
    var idx = parseInt(btn.getAttribute('data-idx'));
    var group = searchGroups[idx];
    if (!group) return;
    
    if (action === 'edit-search-group') {
        showSearchGroupForm(group);
    } else if (action === 'delete-search-group') {
        deleteSearchGroup(group.id);
    }
});

// Event delegation for search configs
document.getElementById('search-list').addEventListener('click', function(e) {
    var btn = e.target.closest('button[data-action]');
    if (!btn) return;
    var action = btn.getAttribute('data-action');
    var idx = parseInt(btn.getAttribute('data-idx'));
    var config = searchConfigsData[idx];
    if (!config) return;
    
    if (action === 'edit-search') {
        showSearchForm(config);
    } else if (action === 'delete-search') {
        deleteSearch(config.id);
    }
});
...
function showSearchGroupForm(group) {
    ...
    showModal('<div class="p-6"><h3 class="text-lg font-bold mb-4">' + (g.id ? '编辑' : '添加') + ' 搜索分组</h3><div class="space-y-3">' +
        '<input type="hidden" id="search-group-id" value="' + escapeHtml(g.id) + '">' +
        '<div><label class="text-sm text-slate-600">分组名称</label>' +
        '<input type="text" id="search-group-name" value="' + escapeHtml(g.name) + '" class="w-full px-3 py-2 border rounded-lg"></div>' +
        '<div><label class="text-sm text-slate-600">描述</label>' +
        '<input type="text" id="search-group-desc" value="' + escapeHtml(g.description || '') + '" class="w-full px-3 py-2 border rounded-lg"></div>' +
...
function showSearchForm(config) {
    ...
    var groupOpts = searchGroups.map(function(g) { return '<option value="' + g.id + '"' + (g.id === c.group_id ? ' selected' : '') + '>' + escapeHtml(g.name) + '</option>'; }).join('');
    ...
    showModal('<div class="p-6"><h3 class="text-lg font-bold mb-4">' + (c.id ? '编辑' : '添加') + ' 搜索引擎配置</h3><div class="space-y-3">' +
        '<input type="hidden" id="search-id" value="' + escapeHtml(c.id) + '">' +
        '<div><label class="text-sm text-slate-600">名称</label><input type="text" id="search-name" value="' + escapeHtml(c.name) + '" class="w-full px-3 py-2 border rounded-lg"></div>' +
        ...
        '<div><label class="text-sm text-slate-600">API Key</label><input type="password" id="search-key" value="' + escapeHtml(c.api_key) + '" class="w-full px-3 py-2 border rounded-lg"></div>' +
