...
function showLLMGroupForm(group) {
    ...
    showModal('<div class="p-6"><h3 class="text-lg font-bold mb-4">' + (g.id ? '编辑' : '添加') + ' LLM 分组</h3><div class="space-y-3">' +
        '<input type="hidden" id="llm-group-id" value="' + g.id + '">' +
        '<div><label class="text-sm text-slate-600">分组名称</label>' +
        '<input type="text" id="llm-group-name" value="' + g.name + '" class="w-full px-3 py-2 border rounded-lg"></div>' +
        '<div><label class="text-sm text-slate-600">描述</label>' +
        '<input type="text" id="llm-group-desc" value="' + (g.description || '') + '" class="w-full px-3 py-2 border rounded-lg"></div>' +
...
function showLLMForm(config) {
    ...
    showModal('<div class="p-6"><h3 class="text-lg font-bold mb-4">' + (c.id ? '编辑' : '添加') + ' LLM 配置</h3><div class="space-y-3">' +
        '<input type="hidden" id="llm-id" value="' + c.id + '">' +
        '<div><label class="text-sm text-slate-600">名称</label><input type="text" id="llm-name" value="' + c.name + '" class="w-full px-3 py-2 border rounded-lg"></div>' +
        ...
        '<div><label class="text-sm text-slate-600">Base URL（可选）</label><input type="text" id="llm-url" value="' + c.base_url + '" placeholder="留空使用默认" class="w-full px-3 py-2 border rounded-lg"></div>' +
        '<div><label class="text-sm text-slate-600">API Key</label><input type="password" id="llm-key" value="' + c.api_key + '" class="w-full px-3 py-2 border rounded-lg"></div>' +
        '<div><label class="text-sm text-slate-600">模型</label><input type="text" id="llm-model" value="' + c.model + '" placeholder="例如: gpt-4o" class="w-full px-3 py-2 border rounded-lg"></div>' +
