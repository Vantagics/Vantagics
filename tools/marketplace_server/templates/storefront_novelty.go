...
template.New("storefront_novelty").Funcs(storefrontFuncMap).Parse(novP1 + novP2 + novP3 + novP4 + novSupportFloat + "\n" + I18nJS + "\n</body>\n</html>"),
)
...
{{if .Storefront.HasLogo}}<meta property="og:image" content="/store/{{.Storefront.PublicID}}/logo" />{{end}}
...
{{if .Storefront.HasLogo}}<meta name="twitter:image" content="/store/{{.Storefront.PublicID}}/logo" />{{end}}
<style>
...
body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Microsoft YaHei",sans-serif;background:var(--cream);min-height:100vh;color:var(--tp);line-height:1.6;-webkit-font-smoothing:antialiased;position:relative;overflow-x:hidden;}
...
.sf-dl-btn-primary:hover{background:linear-gradient(135deg,var(--g600),var(--g700));box-shadow:0 4px 16px rgba(184,148,58,0.35);color:#fff;}.sf-dl-btn svg{width:16px;height:16px;flex-shrink:0;}
...
.btn{padding:9px 20px;border:none;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:6px;text-decoration:none;transition:all 0.25s;font-family:inherit;white-space:nowrap;}
...
.badge-owned{display:inline-flex;align-items:center;gap:6px;padding:8px 16px;background:#f0f5e8;color:#5a7a2e;border:1px solid #d4e4b8;border-radius:10px;font-size:12px;font-weight:700;white-space:nowrap;}.badge-owned svg{width:14px;height:14px;}
...
<div class="store-profile"><div class="store-avatar-ring"><div class="store-avatar">{{if .Storefront.HasLogo}}<img src="/store/{{.Storefront.PublicID}}/logo" alt="{{.Storefront.StoreName}}">{{else}}<div class="store-avatar-letter">{{firstChar .Storefront.StoreName}}</div>{{end}}</div></div>
...
var _storePublicID='{{.Storefront.PublicID}}';
...
const novSupportFloat = `{{if .SupportApproved}}
<style>
.support-float {
...
function enterCustomerSupport() {
    ...
    var storePublicID = '{{.Storefront.PublicID}}';
    if (!isLoggedIn) {
        window.location.href = '/user/login?redirect=' + encodeURIComponent('/store/' + storePublicID + '?support=1');
        return;
    }
